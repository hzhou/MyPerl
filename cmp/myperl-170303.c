...
#define T_and 0x818
#define T_not 0x91d
#define T_FN_LIST 0xa22
#define T_FN_GOTO 0xb22
#define T_Comma 0xc24
#define T_Eq 0xd29
#define T_PlusEq 0xe29
#define T_MinusEq 0xf29
#define T_MultEq 0x1029
#define T_DivEq 0x1129
#define T_ModEq 0x1229
#define T_DotEq 0x1329
#define T_OrEq 0x1429
#define T_AndEq 0x1529
#define T_CtrlEq 0x1629
#define T_GtGtEq 0x1729
#define T_LtLtEq 0x1829
#define T_Colon 0x192d
#define T_Qmark 0x1a2d
#define T_DotDot 0x1b30
#define T_OrOr 0x1c34
#define T_AndAnd 0x1d38
#define T_Or 0x1e3c
#define T_Ctrl 0x1f3c
#define T_And 0x2040
#define T_EqEq 0x2144
#define T_EmarkEq 0x2244
#define T_LtEqGt 0x2344
#define T_eq 0x2444
#define T_ne 0x2544
#define T_cmp 0x2644
#define T_Lt 0x2748
#define T_Gt 0x2848
#define T_LtEq 0x2948
#define T_GtEq 0x2a48
#define T_lt 0x2b48
#define T_gt 0x2c48
#define T_le 0x2d48
#define T_ge 0x2e48
#define T_FN_1 0x2f4e
#define T_GtGt 0x3050
#define T_LtLt 0x3150
#define T_Plus 0x3254
#define T_Minus 0x3354
#define T_Dot 0x3454
#define T_Mult 0x3558
#define T_Div 0x3658
#define T_Mod 0x3758
#define T_x 0x3858
#define T_EqTlide 0x395c
#define T_UMINUS 0x3a62
#define T_Tlide 0x3b62
#define T_Emark 0x3c62
#define T_UAMPER 0x3d62
#define T_MultMult 0x3e65
#define T_PlusPlus 0x3f6a
#define T_MinusMinus 0x406a
#define T_MinusGt 0x416c
#define T_UNARY 0x4272
#define T_SIGIL 0x4372
#define T_MY 0x4472
...
struct CV* perl_parse(char* s_in, FILE* file_in);
    ...
    while(i<modules_len){
        ...
        if(ts_file){
            ...
            if(file_in == NULL){
                ...
                stash_global = g_stashes[0];
                cur_file_scope = g_file_scopes_len;
                cv_module = perl_parse(NULL, file_in);
                if(g_file_scopes_len + 1 > g_file_scopes_size){
                    g_file_scopes = f_darray_expand(g_file_scopes, sizeof(struct call_stack_entry), &g_file_scopes_size);
                }
                g_file_scopes[g_file_scopes_len].sv_local = g_local;
    ...
    while(1){
        ...
        do {
            ...
            if(s == NULL){
                ...
                else if(tn_unary_word == WORD_open ||tn_unary_word == WORD_close ||tn_unary_word == WORD_print ||tn_unary_word == WORD_printf ){
                    ts_var_buf[0] = '*';
                ...
                else if(stack[i_top-1].type == T_BOC && stack[i_top-1].value.n == '<'){
                    ts_var_buf[0] = '*';
                    memcpy(ts_var_buf + 1, s2, n);
                    ts_var_buf[n+1] = '\0';
                    tn_op_type = OP_GLOBAL;
        ...
        while(1){
            if((cur.type & 0xffff00) == 0){
                if((stack[i_top-1].type & 0xffff00) == 0){
                        ...
                        if(stack[i_top-2].type == T_FN_GOTO){
                            ...
                            switch (stack[i_top-2].type){
                                ...
                                case T_MY:
                                    ...
                                    break;
                                case T_SIGIL:
                                    char_sigil = stack[100-2].value.n;
                                    op = get_op();
                                    op->type = OP_SIGIL;
                    ...
                    if(cur.type == T_EOF){
                        if(i_top == 2){
                            if(stack[i_top-1].value.op->type == OP_SEQ){
                                ...
                                stack[i_top-1].value.op->type = OP_block;
                            }
                            cv_cur->op_block = stack[i_top-1].value.op;
                            cv_cur->n_vars = g_local_len - cv_cur->i_start;
                            cv_cur->n_seq = i_seq;
                            cv = cv_cur;
                            lex_stack_len--;
